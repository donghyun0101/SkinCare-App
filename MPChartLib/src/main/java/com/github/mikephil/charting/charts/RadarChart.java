...
public class RadarChart extends PieRadarChartBase<RadarData> {
    ...
    protected RadarChartRenderer mRenderer;
...
//            mYAxisRenderer.computeAxis(mYAxis.mAxisMinimum, mYAxis.mAxisMaximum, mYAxis.isInverted());
    ...
    /**
     * Miru RadarChart 전체를 감싸는 큰 원 그리기
     *
     * @param canvas
     */
    public void bigCircle(Canvas canvas) {
        Paint paint = new Paint();
        MPPointF position = mViewPortHandler.getContentCenter();

        float x = position.x;
        float y = position.y;

//        Log.e("bigcircle x", String.valueOf(x));
//        Log.e("bigcircle y", String.valueOf(y));

        paint.setColor(Color.parseColor("#F2EEEF")); // TODO 큰 원 색상
        paint.setStrokeWidth(getWebLineWidthInner());
        paint.setStyle(Paint.Style.FILL);

        Path path = new Path();
        path.addCircle(0, 0, getRadius(), Path.Direction.CW);

//        Log.e("radius", String.valueOf(getRadius()));
